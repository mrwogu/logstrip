    ...
    document.querySelectorAll('[data-logstrip-compare]').forEach((root) => {
      ...
      root.innerHTML = `
        <div class="logstrip-demo__bar">
          ...
          <span class="logstrip-demo__dot"></span>
          <span class="logstrip-demo__dot"></span>
          <span class="logstrip-demo__title">grove-agent · context run</span>
        </div>
        ...
        <div class="logstrip-demo__screen" data-logstrip-screen>
          ...
          <pre class="logstrip-demo__pre logstrip-demo__pre--loading" data-logstrip-loading-pre>
<span style="color: rgba(164, 255, 175, 0.8);">❯ logstrip raw.log --stats</span>
<span style="color: rgba(247, 244, 234, 0.5); display: inline-block; margin-top: 0.5rem; animation: logstrip-loading-pulse 1.2s infinite ease-in-out;">analyzing noise patterns and compressing context...</span>
          </pre>
          <pre class="logstrip-demo__pre logstrip-demo__pre--clean" data-logstrip-clean-pre></pre>
          <div class="logstrip-demo__agent-card" aria-label="Agent-ready summary">
            <span class="logstrip-demo__agent-kicker">what the agent sees</span>
            ${agentSummary.map(([label, value]) => `<div class="logstrip-demo__agent-row"><b>${esc(label)}</b><span>${esc(value)}</span></div>`).join('')}
          </div>
        </div>
        <div class="logstrip-demo__metrics" data-logstrip-metrics>
          <span><strong data-logstrip-m-savings>${compressed.savingsPercent}%</strong> token savings</span>
          <span><strong data-logstrip-m-dropped>${compressed.droppedLines}</strong> lines dropped</span>
          <span><strong data-logstrip-m-dupes>${compressed.duplicateLines}</strong> duplicates folded</span>
          <span><strong data-logstrip-m-hidden>${compressed.hiddenInternalStackLines}</strong> internal frames hidden</span>
        </div>`;
