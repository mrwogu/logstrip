...
const ACCESS_LOG_LINE_PATTERN =
  /^\S+\s+\S+\s+\S+\s+\[.+\]\s+"(?:GET|POST|PUT|DELETE|HEAD|OPTIONS|PATCH|CONNECT|TRACE)\s+(\S+)\s+HTTP\/\d\.\d"\s+(\d{3})\b/iu;

const ACCESS_LOG_FULL_PATTERN =
  /^\S+\s+\S+\s+\S+\s+\[.+\]\s+"(?:GET|POST|PUT|DELETE|HEAD|OPTIONS|PATCH|CONNECT|TRACE)\s+(\S+)\s+HTTP\/\d\.\d"\s+(\d{3})\s+\d+\s+"[^"]*"\s+"([^"]*)"/iu;

// ---- Noise classification patterns ----

const HEALTH_PATH_PATTERN =
  /\/(?:healthz|readyz|livez|health|ping|status|up)$/iu;

const METRICS_PATH_PATTERN =
  /\/(?:metrics|metricz)$/iu;

const STATIC_ASSET_PATH_PATTERN =
  /\.(?:css|js|mjs|cjs|svg|ico|png|jpe?g|gif|webp|woff2?|ttf|eot|otf|map|txt|xml|json|webmanifest|wasm)(?:\?.*)?$/iu;

const ROBOTS_FAVICON_PATH_PATTERN =
  /\/(?:favicon\.ico|robots\.txt)$/iu;

const KUBE_PROBE_UA_PATTERN =
  /\bkube-probe\b/iu;

const PROMETHEUS_UA_PATTERN =
  /\bPrometheus\b/iu;

const INTERNAL_HEALTH_UA_PATTERN =
  /\b(?:ELB-HealthChecker|GoogleHC|kube-probe|node-fetch|Go-http-client|curl|wget)\b/iu;

export interface AccessLogMatch {
  path: string;
  status: number;
  userAgent: string;
}

export function matchAccessLogLine(line: string): AccessLogMatch | null {
...
export function isAccessLogNoiseLine(line: string): boolean {
...
export function extractHttpStatusFromLine(line: string): number {
