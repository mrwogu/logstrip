...
} from '../src/core/bonsai-config';

const VALID_CONFIG_YAML = `# ContextBonsai custom config for Acme Corp
sources:
  - name: acme-gateway
    markers:
      - acme-gateway
...
`;
  ...
  describe('resolveConfigPath', () => {
    it('returns explicit path when file exists', () => {
      const dir = join(tmpdir(), 'bonsai-config-test-explicit');
      mkdirSync(dir, { recursive: true });
      const filePath = join(dir, '.bonsai.yml');
    ...
    it('returns undefined when explicit path does not exist', () => {
      const result = resolveConfigPath('/nonexistent/.bonsai.yml');
      expect(result).toBeUndefined();
    });

    it('auto-detects .bonsai.yml in startDir', () => {
      const dir = join(tmpdir(), 'bonsai-config-test-autodetect');
      mkdirSync(dir, { recursive: true });
      const filePath = join(dir, '.bonsai.yml');
      writeFileSync(filePath, 'sources: []\n', 'utf8');
    ...
    it('returns undefined when no config found', () => {
      const dir = join(tmpdir(), 'bonsai-config-test-nocfg');
      mkdirSync(dir, { recursive: true });
  ...
  describe('loadBonsaiConfig', () => {
    it('loads and parses a config file', () => {
      const dir = join(tmpdir(), 'bonsai-config-test-load');
      mkdirSync(dir, { recursive: true });
      const filePath = join(dir, '.bonsai.yml');
      writeFileSync(filePath, MINIMAL_CONFIG_YAML, 'utf8');

      const config = loadBonsaiConfig(filePath);
      expect(config.sources).toHaveLength(1);
      expect(config.sources[0].name).toBe('internal-tool');
    });

    it('returns empty config when no path resolves', () => {
      const config = loadBonsaiConfig('/nonexistent/.bonsai.yml');
      expect(config.sources).toHaveLength(0);
    });

    it('loads config from auto-detected startDir', () => {
      const dir = join(tmpdir(), 'bonsai-config-test-autodetect-load');
      mkdirSync(dir, { recursive: true });
      writeFileSync(join(dir, '.bonsai.yml'), MINIMAL_CONFIG_YAML, 'utf8');

      const config = loadBonsaiConfig(undefined, dir);
      expect(config.sources).toHaveLength(1);
      expect(config.sources[0].name).toBe('internal-tool');
    });
  });
