...
Product order of priority:

1. **CLI** (`bonsai`, `context-bonsai`) - canonical user-facing entry. Unix
conventions: stdin/stdout by default, `-o/--output`, `-a/--aggressiveness`,
`--stats` on stderr, `--json` on stdout (requires `--output`).

2. **Library** (`import { processLogFile } from 'context-bonsai'`) - embed
entry for custom Node tooling.

3. **GitHub Action** - optional thin wrapper for one-step CI integration.
Prefer the CLI in `run:` steps when possible.
...
- Never break the public surface exported from `src/cli/index.ts`,
`src/core/bonsai-parser.ts`, or the GitHub Action wired in `action.yml`

When the user requests new behavior, first explore the existing CLI, parser,
tests, fixtures and CI workflow before proposing code. Match the existing
style: small pure helpers, named exports, only `node:*` built-ins on the
CLI/parser hot path. `@actions/*` packages stay scoped to `src/action/`.

## Tech Stack

typescript, Node.js >= 20 (CLI uses node:util.parseArgs; action.yml uses node20)
...
- layout: Unit tests next to source coverage live in tests/bonsai-parser.test.ts, tests/cli.test.ts, tests/action.test.ts, End-to-end smoke tests live in tests/smoke.test.ts and operate on real fixtures, Fixtures live in tests/fixtures/\*.log; snapshots in tests/fixtures/\_\_snapshots\_\_/\*.bonsai.snap
- rules: Coverage thresholds are 100/100/100/100 (vitest.config.ts) - do not lower them, When adding a new fixture, profile it first to set realistic minSavingsPercent and mustContain/mustNotContain assertions, Snapshots are committed - regenerate intentionally with vitest -u and review the diff, Mock @actions/core and @actions/github in action tests; never hit the real GitHub API, Mock CLI IO via the CliIo interface (stdin/stdout/stderr/stdinIsTTY); never touch process.stdout directly in tests
- ci.yml runs on Node 20.x / 22.x / 24.x; keep parser and CLI compatible with all three
- The CLI smoke step in CI feeds a printf-built log through node dist/cli/index.js and greps for [x2] [ERROR]
...
```
/review    - Review the staged diff against the rules in @standards and @restrictions, then summarise findings in a markdown table (Severity | File | Finding | Status).
/test      - Add or update vitest tests so coverage stays at 100%. Place unit cases in tests/bonsai-parser.test.ts, tests/cli.test.ts, or tests/action.test.ts; place end-to-end checks in tests/smoke.test.ts.
/cli       - Walk through src/cli/index.ts: parseCliOptions (flags + aggressiveness), runCli (stdin/stdout/file routing, error mapping), formatStats. Mention the canonical I/O contract: stats on stderr, JSON on stdout, content on stdout-or-output.
/fixture   - Create a new realistic CI log in tests/fixtures/<name>.log, profile its parser output, and add a smoke case (case object + snapshot) in tests/smoke.test.ts. Snapshot file is tests/fixtures/\_\_snapshots\_\_/<name>.bonsai.snap.
/verify    - Run npm run typecheck && npm run test:coverage && npm run build && npm audit && npm run docs:build. Report results in a status line.
/compress  - Use the CLI (bonsai or node dist/cli/index.js) to compress a log path the user provides and summarise the resulting BonsaiResult (input/output tokens, savings %, droppedLines, duplicateLines, hiddenInternalStackLines).
/parser    - Walk through src/core/bonsai-parser.ts: regex tables, shouldKeepLine flow, processLogStream loop, sanitizeLine, internal-stack collapsing, deduplication. Reference line numbers.
/limits    - List the documented limitations from @context (UUID v6+, Java/Python stack patterns, compound exception names, aggressiveness placeholder, internal-stack pattern gaps) so the user can pick one to fix.
```
